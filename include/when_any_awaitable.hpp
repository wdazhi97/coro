...
#ifndef COROUTINE_WHEN_ANY_AWAITABLE_HPP
#define COROUTINE_WHEN_ANY_AWAITABLE_HPP
#include "macro.h"
#include "when_any_counter.hpp"
...
// 可变参数模板特化
    ...
    public:
        ...
        explicit when_any_ready_awaitble(Tasks&& ...tasks)
        noexcept(std::conjunction_v<std::is_nothrow_move_constructible<Tasks>...>)
                :counter_(sizeof...(Tasks))
                ,tasks_(std::move(tasks)...)
        {}

        explicit when_any_ready_awaitble(std::tuple<Tasks ...> &&tasks)
                :counter_(sizeof...(Tasks))
                ,tasks_(std::move(tasks))
        {}
...
// 容器类型特化
    ...
    {
    public:
        explicit when_any_ready_awaitble(TaskContainer&& tasks)
                : counter_(tasks.size())
...
__CPP_CORO_NS_END
#endif //COROUTINE_WHEN_ANY_AWAITABLE_HPP

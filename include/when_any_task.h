...
#ifndef COROUTINE_WHEN_ANY_TASK_H
#define COROUTINE_WHEN_ANY_TASK_H
#include "macro.h"
...
// task定义
    ...
    when_any_task<Result> make_when_all_task(Awaitable awaitable)
    {
    ...
    template<typename Awaitable,
            typename Result = typename awaitable_traits<Awaitable &&>::await_result_t,
            std::enable_if_t<!std::is_void_v<Result>, int> = 0>
    when_any_task<Result> make_when_all_task(std::reference_wrapper<Awaitable> awaitable)
    {
...
__CPP_CORO_NS_END
#endif //COROUTINE_WHEN_ANY_TASK_H
